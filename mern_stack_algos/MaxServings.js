...
function getMaxServings(recipe, available) {
    ...
    for (const requiredIngredient in recipe) {
        const availableAmount = available[requiredIngredient]
        const required = recipe[requiredIngredient]
        if (!availableAmount || availableAmount < required) {
            return 0
        }
        const temp = availableAmount / required
        if (temp < limit) {
            limit = temp
        }
    }
...
console.log(getMaxServings(recipe1, available1), expected1)
console.log(getMaxServings(recipe1, available2), expected2)
console.log(getMaxServings(recipe1, available3), expected3)