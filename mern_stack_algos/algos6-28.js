...
function groupObjects2(items, key) {
    let result = {}

    for (let obj of items) {

        let compare = obj[key].toLowerCase()
        
        if (result.hasOwnProperty(compare)){
            result[compare].push(obj)
        } else {
            result[compare] = []
            result[compare].push(obj)
        }
...
console.log(groupObjects2(objects, 'name'))